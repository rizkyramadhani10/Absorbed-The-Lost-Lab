...
# 🔒 Stage minimal yang harus dicapai agar tali bisa diambil
@export var required_stage: GameState.StoryStage = GameState.StoryStage.BACK_TO_LAB 

# 🔥 TAMBAHAN: Variabel untuk memajukan progres cerita setelah tali diambil
...
func _ready() -> void:
	# Cek apakah tali sudah diambil sebelumnya
	if Global.has_rope == true:
		# 🔥 Tunda 1 frame agar Node lain (seperti LevelManager) selesai _ready()
		await get_tree().process_frame
		
		# 🔥 Majukan progres cerita jika tali sudah pernah diambil tapi stage masih tertinggal
...
func _on_body_entered(body: Node2D) -> void:
	if body.name == "Player" or body.is_in_group("player"):
		...
		if GameState.current_stage < required_stage:
			print("Tali dikunci. Butuh stage: ", required_stage)
			return
