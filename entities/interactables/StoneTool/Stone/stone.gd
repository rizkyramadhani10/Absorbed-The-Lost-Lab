...
# 🔒 Stage minimal yang harus dicapai agar batu bisa diambil
@export var required_stage: GameState.StoryStage = GameState.StoryStage.BACK_TO_LAB 

# 🔥 TAMBAHAN: Variabel untuk memajukan progres cerita setelah batu diambil
...
func _ready() -> void:
	# Cek apakah batu sudah diambil sebelumnya
	if Global.has_stone == true:
		# 🔥 Tunda 1 frame agar Node lain (seperti LevelManager) selesai _ready()
		await get_tree().process_frame
		
		# 🔥 Majukan progres cerita jika batu sudah pernah diambil tapi stage masih tertinggal
...
func _on_body_entered(body: Node2D) -> void:
	if body.name == "Player" or body.is_in_group("player"):
		...
		if GameState.current_stage < required_stage:
			print("Batu dikunci. Butuh stage: ", required_stage)
			return
