...
func _unhandled_input(event: InputEvent) -> void:
	if player_nearby and event.is_action_pressed("interact") and not is_dialog_playing:
		...
		# 🚪 JIKA SUDAH MELEWATI SHOWER -> PROSES PINDAH SCENE
...
func _play_locked_dialog() -> void:
	if locked_dialog_path == "":
		print("ERROR di ExitDoor: 'Locked Dialog Path' di Inspector masih KOSONG!")
		return
	...
	if dp == null:
		print("ERROR di ExitDoor: Node 'DialogPlayer' tidak ditemukan pada " + player_ref.name)
		return
